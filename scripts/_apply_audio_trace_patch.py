...
ENGINE = Path("mpclab/engine.py")
TEST = Path("tests/test_engine_audio_trace.py")
TEMP_FILES = (
    Path(".github/workflows/apply-audio-trace-patch.yml"),
    Path(".github/workflows/trigger-audio-trace-patch.yml"),
    Path(".github/workflows/trigger-audio-trace-patch-v2.yml"),
    Path("scripts/_apply_audio_trace_patch.py"),
)
...
pad_cut_start = text.index("    def _pad_trigger_cuts(\n")
pad_cut_end = text.index("    def _spawn_synth(\n", pad_cut_start)
text = (
    text[:pad_cut_start]
    + '''    def _pad_cut_reason(
...
for temp in TEMP_FILES:
    if temp != Path("scripts/_apply_audio_trace_patch.py"):
