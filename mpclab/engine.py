...
class Engine:
    ...
    def _spawn_audio_clip(self, clip, offset: int, elapsed_beats: float = 0.0) -> None:
        ...
        # An Arrange audio block is a non-destructive time stretch: its
        # timeline length defines the output duration, while its source trim
        # remains unchanged. Loop blocks deliberately retain their natural
        # speed and repeat that trim instead.
        rate = 1.0 if clip.loop else (s1 - s0) / arranged
        length = arranged
