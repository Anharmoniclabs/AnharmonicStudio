...
def decode_audio(source, destination, cancel, *, rate=SAMPLE_RATE, channels=1):
    ...
    command = [ffmpeg, "-v", "error", "-nostdin", "-y", "-i", str(source),
               "-map", "0:a:0", "-t", str(MAX_SECONDS + 1), "-ar", str(rate),
               "-ac", str(channels), "-c:a", "pcm_f32le", str(destination)]
    check_cancel(cancel)
    with tempfile.TemporaryFile() as errors:
        ...
        process = subprocess.Popen(command, stdout=subprocess.DEVNULL, stderr=errors,
                                   env=external_environment(ffmpeg), **subprocess_options())
        try:
            while True:
                check_cancel(cancel)
                try:
            ...
            if code:
                errors.seek(0)
                raise ValueError(errors.read(1500).decode(errors="replace") or "Could not decode this audio file.")
        finally:
...
def decode_activations(frames, onsets, duration, *, threshold=.3, minimum=.1, cancel=None):
    ...
    for column in range(88):
        ...
        for index, strength in enumerate(frames[:, column]):
            ...
            if start is not None and onset and index - start >= minimum_frames:
                ...
                start = None
            if strength >= threshold or (onset and strength >= threshold * .6):
                if start is None:
                    start = index
                last_active = index
...
def transcribe_pitched(path, session, cancel, progress, *, threshold=.3, minimum=.1):
    ...
    with sf.SoundFile(path) as audio:
        ...
        for offset in range(0, length + OVERLAP // 2, HOP):
            ...
            if not np.isfinite(chunk).all():
                raise ValueError("Audio contains non-finite samples.")
            note, onset = session.run(["StatefulPartitionedCall:1", "StatefulPartitionedCall:2"],
    ...
    notes = decode_activations(np.concatenate(frames)[:count], np.concatenate(onsets)[:count],
                               duration, threshold=threshold, minimum=minimum, cancel=cancel)
    ...
    floor = max(1e-6, signal_level(path, cancel) * .02)
    audible = []
    with sf.SoundFile(path) as audio:
        for note in notes:
            check_cancel(cancel)
...
def separate_stems(source, directory, model_name, cancel, progress):
    ...
    try:
        ...
        raise RuntimeError("Song separation needs the optional stem engine. Run ./install-separation.sh, "
                           "or choose Single instrument to transcribe without separation.") from exc
    check_cancel(cancel)
    ...
    try:
        with sf.SoundFile(decoded) as audio:
            ...
            for name, path in paths.items():
                handles[name] = sf.SoundFile(path, "w", samplerate=rate,
                                            channels=model.audio_channels, subtype="FLOAT")
            for offset in range(0, len(audio), chunk_size):
                check_cancel(cancel)
                start = max(0, offset - context)
                audio.seek(start)
                data = audio.read(min(chunk_size + context + offset - start, len(audio) - start),
                                  dtype="float32", always_2d=True)
                wave = torch.from_numpy(data.T.copy())
                ref = wave.mean(0)
                mean, std = ref.mean(), ref.std().clamp(min=1e-6)
                with torch.inference_mode():
                    output = apply_model(model, ((wave - mean) / std)[None],
                                         device="cpu", shifts=0, split=True, overlap=.25,
...
def transcribe_drums(path, cancel):
    ...
    families = {}
    mapping = {"kick": ("Kick", 36), "snare": ("Snare", 38),
               "hat": ("Hi-hat", 42), "hat-open": ("Hi-hat", 46),
               "clap": ("Snare", 39), "cymbal": ("Cymbals", 49)}
    with sf.SoundFile(path) as audio:
        ...
        for offset in range(0, len(audio), block):
            ...
            for hit in find_hits(chunk, audio.samplerate):
                ...
                part.notes.append(DetectedNote(pitch, start, min(start + .12, len(audio) / audio.samplerate),
                                               float(hit.score)))
    return list(families.values())


def transcribe_file(source, *, mode="song4", bpm=None, threshold=.45, minimum=.1,
    ...
    if not .1 <= threshold <= .9 or not .05 <= minimum <= 1:
    ...
    with tempfile.TemporaryDirectory(prefix="anharmonic-transcription-") as temporary:
        ...
        if mode == "single":
            ...
            paths = separate_stems(source, directory, "htdemucs_6s" if mode == "song6" else "htdemucs",
                                   cancel, lambda text, value: progress(text, .05 + value * .5))
        parts = []
        labels = {"vocals": "Vocals", "bass": "Bass", "other": "Other instruments",
        ...
        for index, (name, path) in enumerate(paths.items()):
            ...
            if mode != "single" and signal_level(path, cancel) < max(1e-6, source_level * .01):
                continue
            base = .55 + index / len(paths) * .4
            ...
            if path != mono:
                ...
                decoded = mono
            notes = transcribe_pitched(decoded, session, cancel,
                                       lambda value, base=base, name=name: progress(f"Detecting notes · {name}",
                                                                         base + value / len(paths) * .4),
        ...
        if result.note_count > MAX_NOTES:
            raise ValueError("Too many detected notes. Increase the confidence threshold or shorten the source.")
        check_cancel(cancel)
