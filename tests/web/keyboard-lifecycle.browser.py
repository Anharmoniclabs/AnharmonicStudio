...
def main():
    ...
    parser.add_argument('--root', type=Path, default=Path(__file__).resolve().parents[2], help='Checkout to test')
        ...
        try:
            for workspace in ('notes', 'instruments'):
                for scenario in ('blur', 'release-during-startup'):
                    ...
                    results.append({'name': f'{workspace}: {scenario}', 'passed': not active,
                                    'active_notes': active, 'events': page.evaluate('voiceLog')})
                    context.close()
        finally:
            browser.close()
