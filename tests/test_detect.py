...
class PreciseChopTests(unittest.TestCase):
    def test_chops_follow_swung_attacks_without_grid_quantization(self):
        wave = np.zeros(SR * 5, dtype=np.float32)
        expected = [0.173, 0.487, 0.91, 1.253, 1.719, 2.111, 2.532, 3.073]
        sounds = (kick(), snare(), hat())
        for i, at in enumerate(expected):
            place(wave, sounds[i % 3] * (0.12 if i % 2 else 1.0), at)
        result = detect.scan(wave, bpm=120)
        for at in expected:
            self.assertLess(min(abs(t - at) for t in result["onsets"]), 0.003, at)
        self.assertTrue(all(c.end <= len(wave) / SR for c in result["hits"]))

    def test_similar_timbre_with_different_rhythm_is_not_a_repeated_loop(self):
        wave = np.zeros(SR * 5, dtype=np.float32)
        for at in (0, 0.5, 1, 1.5, 2.25, 2.75, 3.25, 3.75):
            place(wave, kick(), at)
        self.assertEqual(detect.find_loops(wave, bpm=120, phase=0, bars=(1,)), [])

    def test_long_pattern_tempo_does_not_accumulate_frame_rounding_drift(self):
        result = detect.scan(build_pattern(bars=32))
        self.assertAlmostEqual(result["bpm"], 120, delta=0.03)
        self.assertTrue(result["loops"])
        self.assertTrue(any(c.detail["content"] == "drum break" for c in result["loops"]))
        self.assertLess(min(result["phase"], 0.5 - result["phase"]), 0.005)


def test_known_instrument_stem_context_is_preserved():
    audio = build_pattern(bars=8)
    for stem, expected_hit, expected_loop in (
        ("bass", "bass", "bass phrase"),
        ("other", "tonal", "instrument phrase"),
        ("vocals", "tonal", "vocal phrase"),
    ):
        result = detect.scan(audio, bpm=120, source_kind=stem)
        assert result["hits"]
