(() => {
  ...
  function renderMix() {
    ...
    shell('Mixer', '<span class="bars-readout">' + tracks.length + ' stereo tracks → Master</span>' + button('+ Mixer track', 'add-mixer-track'),
    ...
    on('.add-mixer-track', 'click', () => {
      if (state.playing || state.recording || state.recordPending) throw new Error('Stop playback and finish the recording before adding a mixer track.');
      projectStore.addTrack(); renderMix(); setStatus('Mixer track added. Assign pads, synth or audio clips to its channel.');
    });
    $('.add-mixer-track').disabled = tracks.length >= window.AnharmonicProject.MAX_TRACKS;
    ...
    if (state.workspace === 'instruments' || state.workspace === 'notes') {
      ...
      if (pitch !== undefined) {
        ...
        if (state.workspace === 'instruments') {
          ...
          const engine = await ensureAudio(); engine?.triggerNote(pitch, { pad: targetPad, velocity: .8 });
        }
        return;
      }
    }
